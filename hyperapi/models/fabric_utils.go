...
func fabric_query_codes() []LuaChaincode {
	...
	invokeArgs := [][]byte{[]byte("")}
	value, _, err := chClient.Execute(apitxn.Request{ChaincodeID: ccID, Fcn: "getAllChaincodes", Args: invokeArgs})
	if err != nil {
		fmt.Println("Failed to query values: %s", err)
	}
	err = json.Unmarshal(value, &codeStores)
	if err != nil {
		fmt.Println("Failed to unpack values: %s", err)
	}
	fmt.Println("response value: ", string(value))
	...
	if string(value) != "null" {
		for key, value := range codeStores {
			...
			for keyTarget, valueTarget := range codeStores[key].Target {
				luaCode.Targets = append(luaCode.Targets, keyTarget)
				luaCode.Validations = append(luaCode.Validations, valueTarget)
			}
